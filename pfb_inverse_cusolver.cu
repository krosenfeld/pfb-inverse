...
int inverse_pfb(cufftComplex *d_s, int num_samples, int num_tap, int num_freq, float *d_rts){
  ...
  int *d_csrRowPtrBandPPT = NULL;
  int *d_csrColIndBandPPT = NULL;
  float *d_csrValBandPPT = NULL;
  float *csrValBandPPT, *bandP;
  int *csrRowPtrBandPPT,*csrColIndBandPPT;
  ...
  int m = nblock;
  int nnz = 2 * (num_tap*nblock - (num_tap - 1) * num_tap / 2) - nblock;
  csrRowPtrBandPPT = (int *) malloc((m + 1)*sizeof(int));
  csrColIndBandPPT = (int *) malloc(nnz*sizeof(int));
  csrValBandPPT = (float *) malloc(lblock*nnz*sizeof(float));
    ...
    for (int j=0; j < m; j++){
      ...
      for (int i=0; i < m; i++){
        ...
        if ((u-dji) >= 0) {
          ...
          csrValBandPPT[nnz*batchId + ind] = val;
          ++ind;
        }
      }
    }
    csrRowPtrBandPPT[m] = nnz;
  }
  ...
  CUDA_CALL( cudaMalloc((void**)&d_csrValBandPPT, sizeof(float)*nnz*lblock) );
  CUDA_CALL( cudaMalloc((void**)&d_csrColIndBandPPT, sizeof(int)*nnz) );
  CUDA_CALL( cudaMalloc((void**)&d_csrRowPtrBandPPT, sizeof(int)*(m+1)) );
  ...
  CUDA_CALL( cudaMemcpy(d_csrValBandPPT, csrValBandPPT, sizeof(float)*nnz*lblock, cudaMemcpyHostToDevice) );
  CUDA_CALL( cudaMemcpy(d_csrColIndBandPPT, csrColIndBandPPT, sizeof(int)*nnz, cudaMemcpyHostToDevice) );
  CUDA_CALL( cudaMemcpy(d_csrRowPtrBandPPT, csrRowPtrBandPPT, sizeof(int)*(m+1), cudaMemcpyHostToDevice) );
